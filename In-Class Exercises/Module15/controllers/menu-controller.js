...
exports.getMenu = async (req, res, next) => {
    ...
    try {
        ...
        console.log("Items:", items);
        if (items) {
            res.render("menu", { pageTitle: "Menu", category: selectedCategory, items: items, categories: categories, isAuthenticated: req.session.isLoggedIn });
        } else {
            res.send("<h1>Oops!  No page found.</h1>")
        }
    } catch (err) {
...
exports.getMenuItem = (req, res, next) => {
    ...
    Category.find()
        ...
        .then((item) => {
            ...
            if (item) {
                res.render("menu-item", { pageTitle: item.name, category: selectedCategory, item: item, categories: categories, isAuthenticated: req.session.isLoggedIn });
            } else {
